...
$filters = $filters ?? [];
    ...
    $donutGradient = $typeDonutGradient ?? 'conic-gradient(#111827 0% 45%, #4b5563 45% 75%, #d1d5db 75% 100%)';
...
<head>
    ...
    <title>Rapports & Statistiques - CEET Incidents</title>
...
<body class="ceet-reports-admin-page">
    <div class="ceet-reports-admin-shell">
        <aside class="ceet-reports-admin-sidebar">
            <div class="ceet-reports-admin-brand">
                <div class="ceet-reports-admin-brand-logo">
            ...
            <nav class="ceet-reports-admin-nav" aria-label="Navigation administrateur">
                @include('partials.ceet-role-nav', ['linkClass' => 'ceet-reports-admin-nav-link'])
            </nav>

            <div class="ceet-reports-admin-sidebar-user">
                <div class="ceet-reports-admin-sidebar-user-main">
                    <div class="ceet-reports-admin-avatar">{{ $currentInitials }}</div>
                ...
                <form action="{{ $safeRoute('logout', [], '#') }}" method="POST" class="ceet-reports-admin-logout-form">
                    @csrf

                    <button type="submit" class="ceet-reports-admin-logout-button">
        ...
        <header class="ceet-reports-admin-topbar">
            <form action="{{ $safeRoute('reports.index', [], '/reports') }}" method="GET" class="ceet-reports-admin-search" data-ceet-reports-filter-form>
            ...
            <div class="ceet-reports-admin-top-actions">
                <a href="{{ $safeRoute('notifications.index', [], '#') }}" class="ceet-reports-admin-icon-btn" aria-label="Notifications">
                    <span class="material-symbols-outlined" aria-hidden="true">notifications</span>
                    <span class="ceet-reports-admin-notification-dot"></span>
                </a>

                <a href="{{ $safeRoute('profile.edit', [], '/profile') }}" class="ceet-reports-admin-icon-btn" aria-label="Aide">
                    <span class="material-symbols-outlined" aria-hidden="true">help_outline</span>
                </a>

                <div class="ceet-reports-admin-top-divider"></div>

                <div class="ceet-reports-admin-top-user">
                    <span>{{ $currentUserName }}</span>
                    <div class="ceet-reports-admin-avatar is-small">{{ $currentInitials }}</div>
                </div>
            </div>
        </header>

        <main class="ceet-reports-admin-main" data-ceet-reports-page>
            <section class="ceet-reports-admin-page-header">
                <div>
                    <h2>Rapports & Statistiques</h2>
                    <p>Analyse de la performance et des incidents du réseau électrique.</p>
                </div>

                <div class="ceet-reports-admin-header-actions">
                    <a class="ceet-reports-admin-secondary-btn" href="{{ $safeRoute('reports.monthly', array_merge($exportQuery, ['format' => 'pdf']), '#') }}">
                        <span class="material-symbols-outlined" aria-hidden="true">picture_as_pdf</span>
                        Exporter en PDF
                    </a>

                    <a class="ceet-reports-admin-primary-btn" href="{{ $safeRoute('reports.monthly', array_merge($exportQuery, ['format' => 'excel']), '#') }}">
            ...
            @if(session('success') || session('status'))
                <div class="ceet-reports-admin-alert is-success" role="status">
                    {{ session('success') ?? session('status') }}
                </div>
            @endif

            @if(session('error'))
                <div class="ceet-reports-admin-alert is-danger" role="alert">
                    {{ session('error') }}
                </div>
            @endif

            <section class="ceet-reports-admin-filter-panel" data-ceet-reports-filter-panel>
                <form method="GET" action="{{ $safeRoute('reports.index', [], '/reports') }}" class="ceet-reports-admin-filter-form" data-ceet-reports-filter-form>
                    ...
                    <div class="ceet-reports-admin-filter-actions">
                        <button type="submit" class="ceet-reports-admin-primary-btn">Appliquer</button>
                        <a href="{{ $safeRoute('reports.index', [], '/reports') }}" class="ceet-reports-admin-secondary-btn">Réinitialiser</a>
                    </div>
                </form>
            </section>
            ...
            <section class="ceet-reports-admin-kpi-grid" aria-label="Indicateurs reporting">
                @foreach($metricCards as $card)
                    <article class="ceet-reports-admin-kpi-card">
            ...
            <section class="ceet-reports-admin-chart-grid">
                <article class="ceet-reports-admin-panel">
                    <header>
                ...
                <article class="ceet-reports-admin-panel">
                    ...
                    <div class="ceet-reports-admin-donut-layout">
                        ...
                        <div class="ceet-reports-admin-legend">
                            @forelse($byType->take(4) as $index => $type)
                                <div>
                                    <i></i>
                                    <span>{{ $type['label'] ?? '-' }} ({{ number_format((int) ($type['total'] ?? 0), 0, ',', ' ') }})</span>
                                </div>
                            @empty
                                <div><i></i><span>Aucune donnée disponible</span></div>
                            @endforelse
                        </div>
                    </div>
                </article>
            ...
            <section class="ceet-reports-admin-lower-grid">
                <article class="ceet-reports-admin-panel">
                    ...
                    <div class="ceet-reports-admin-cause-list">
                        ...
                        @empty
                            <div class="ceet-reports-admin-empty">Aucune cause disponible pour cette période.</div>
                        @endforelse
                    </div>
                </article>
                ...
                <article class="ceet-reports-admin-panel">
                    ...
                    <div class="ceet-reports-admin-table-wrap">
                        <table class="ceet-reports-admin-table">
                            <thead>
                                <tr>
                                    <th>Code</th>
                                    <th>Départ</th>
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5"><div class="ceet-reports-admin-empty">Aucun départ critique détecté.</div></td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <section class="ceet-reports-admin-panel">
                <header>
            ...
            <section class="ceet-reports-admin-export-grid">
                <form method="GET" action="{{ $safeRoute('reports.monthly', [], '#') }}" class="ceet-reports-admin-export-box">
                    ...
                    <div>
                        <button name="format" value="pdf" class="ceet-reports-admin-secondary-btn">PDF</button>
                        <button name="format" value="excel" class="ceet-reports-admin-primary-btn">Excel</button>
                    </div>
                </form>

                <form method="GET" action="{{ $safeRoute('reports.daily', [], '#') }}" class="ceet-reports-admin-export-box">
