...
$userName = $currentUser?->name ?? 'Utilisateur';
    ...
    $code = $incident->code_incident ?: 'INC-' . str_pad((string) $incident->id, 5, '0', STR_PAD_LEFT);
...
<head>
    ...
    <title>{{ $code }} - CEET Incidents</title>
...
<body class="ceet-admin-dashboard-page ceet-incident-show-page">
    <div class="ceet-admin-shell" data-admin-dashboard data-incident-show-page>
        <div class="ceet-dashboard-overlay" data-dashboard-overlay></div>

        <aside class="ceet-admin-sidebar" data-dashboard-sidebar>
            <div class="ceet-admin-brand ceet-incident-operator-brand">
                <div class="ceet-incident-operator-brand-logo">
            ...
            <div class="ceet-admin-sidebar-user ceet-incident-sidebar-user">
        ...
        <header class="ceet-admin-topbar">
            ...
            <form action="{{ $searchRoute }}" method="GET" class="ceet-admin-search">
                <span class="material-symbols-outlined">search</span>
                <input type="search" name="q" value="{{ request('q') }}" placeholder="Rechercher un incident..." autocomplete="off">
            </form>
                        ...
                        <div class="ceet-timeline">
                            @forelse ($actions as $action)
                                @php
                                    ...
                                    $actionTitle = str($action->action_type ?: 'Mise à jour')->replace('_', ' ')->title();
                                @endphp
                                <div class="ceet-timeline-item">
                                    <i></i>
                                    <div>
                                        <header><strong>{{ $actionTitle }}</strong><time>{{ $actionDate?->format('H:i') ?? '--:--' }}</time></header>
                                        <p>{{ $action->description ?: 'Action enregistrée sur cet incident.' }}</p>
